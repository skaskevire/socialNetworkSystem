    ...
    public void init()
    {
		...
		for (int i = 0; i < n; i++) {
			...
			ResponseEntity<String> responseEntity =
		            restTemplate.postForEntity("/camel/users/add", user, String.class);
		...
		for (int i = 0; i < n * 2; i++) {
			...
			ResponseEntity<String> responseEntity =
		     restTemplate.postForEntity("/camel/users/"+user1.getName()+"/friends/add/"+user2.getName(), null, String.class);
			 restTemplate.postForEntity("/camel/users/"+user2.getName()+"/friends/invitations/accept/"+user1.getName(),null,String.class);
		}
		...
		for (int i = 0; i < n * 5; i++) {
			...
			restTemplate.postForEntity("/camel/users/"+user1.getName()+"/messages/post",new Message("msg"),String.class);
    ...
    public void testUserOperationsPerformance() {
		...
		for(User u: users)
		{
			long start = System.currentTimeMillis();
		   	ResponseEntity<List> response = restTemplate.getForEntity("/camel/users/"+u.getName()+"/friends/explore/users", List.class);
		...
		{
			long start = System.currentTimeMillis();
		   	ResponseEntity<List> response = restTemplate.getForEntity("/camel/users/"+u.getName()+"/messages/friends", List.class);
		   	long end = System.currentTimeMillis();
		   	System.out.println(end - start);
		   	Assert.assertTrue((end - start) < tolerance);
		...
		for(User u: users)
		{
			...
			long start = System.currentTimeMillis();
		   	ResponseEntity<List> response = restTemplate.getForEntity("/camel/users/find?name={name}&city={city}", List.class, args);
